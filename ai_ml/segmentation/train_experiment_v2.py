...
def run_experiment_v2(epochs=10, batch_size=16, patience=3):
    set_seed(42)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Starting Experiment 2 training on device: {device}")
    ...
    checkpoint_save_path = os.path.join(exp_model_dir, "improved_unet_v2_best.pth")
    print(f"Experimental Checkpoint path: {checkpoint_save_path}")
    
    # Verify not overwriting baseline
    ...
    for epoch in range(epochs):
        ...
        # Checkpoint based on Validation Loss (lower is better)
    ...
    # Save training log
    csv_path = os.path.join(exp_csv_dir, "v2_training_history.csv")
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=history[0].keys())
        writer.writeheader()
        writer.writerows(history)
        
    # 5. Load best experimental v2 model and run holdout evaluation
    print("\nRunning final Holdout Evaluation using the best Experiment 2 checkpoint...")
    ...
    for t in thresholds:
        ...
        if abs(t - 0.5) < 0.01:
            # Save v2 0.50 metrics separately
            final_metrics_path = os.path.join(exp_csv_dir, "v2_metrics.csv")
            ...
            confusion_matrix_path = os.path.join(exp_csv_dir, "v2_confusion_matrix.csv")
    ...
    threshold_comparison_path = os.path.join(exp_csv_dir, "v2_threshold_comparison.csv")
    ...
    print(f"Experiment 2 evaluation outputs successfully written to: {exp_csv_dir}")

if __name__ == "__main__":
    run_experiment_v2(epochs=10, batch_size=16, patience=3)
